    ...
    <section id="content">
        ...
        <main>
            ...
            <div class="card col-12 col-md-6">
                <div class="card-body">
                    <form action="{{ route('expert.domaines.update', ['id' => $domaine->id]) }}" method="POST">
                        @csrf
                        @if (isset($domaine))
                            @method('PUT')
                        @endif
                        ...
                        <div class="mb-3">
                            ...
                            <select name="service_id" class="form-control shadow-none" id="service_id"
                                {{ isset($domaine) ? 'disabled' : '' }}>

                                <option value="{{ $domaine->service_id }}"
                                   >
                                    {{ $domaine->service->nom }}</option>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" min="0" name="nbr_annee_d_exp" class="form-control shadow-none"
                                id="nbr_annee_d_exp"
                                value="{{ old('nbr_annee_d_exp', isset($domaine) ? $domaine->nbr_annee_d_exp : '') }}">
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="prix_par_jour" class="form-label">Prix par jour(MDH)</label>
                            <input type="text" name="prix_par_duree" class="form-control shadow-none"
                                id="prix_par_duree"
                                value="{{ old('prix_par_duree', isset($domaine) ? $domaine->prix_par_duree : '') }}">
                        </div>

                        <div class="mb-3">
                            <label for="ville" class="form-label">Ville</label>
                            <input type="text" name="ville" class="form-control shadow-none" id="ville"
                                value="{{ old('ville', isset($domaine) ? $domaine->ville : '') }}">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status_" class="form-control shadow-none" id="status_">
                                <option value="active"
                                    {{ old('status_', isset($domaine) && $domaine->status_ == 'active' ? 'selected' : '') }}>
                                    Active</option>
                                <option value="non active"
                                    {{ old('status_', isset($domaine) && $domaine->status_ == 'non active' ? 'selected' : '') }}>
                                    Non Active</option>
                            </select>
                        </div>

                        <button type="submit"
                            class="btn btn-primary">{{ isset($domaine) ? 'Update' : 'Submit' }}</button>
                    </form>
                </div>
